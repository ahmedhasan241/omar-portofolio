export const NAV_LINKS = [
    {
    href: '#about',
    label: 'About me ',
  },
  {
    href: '#Brands',
    label: 'Brands',
  },
  {
    href: '#Tools',
    label: 'Tools',
  },
  {
    href: '#Campaigns',
    label: 'Campaigns',
  },
  // {
  //   href: '#',
  //   label: 'About me ',
  // },
  {
    href: '#contact',
    label: 'Contact',
  },
]

export const HERO_SECTION =
  'Lorem ipsum dolor sit amet consectetur adipisicing elit. Eligendi beatae odio commodi amet minima officia in impedit quibusdam eos iste provident explicabo mollitia doloremque Lorem, ipsum dolor.? Lorem here.'

export const ABOUT_SECTION = {
  section1:
    'Lorem ipsum dolor sit amet consectetur adipisicing elit. Exercitationem magnam ipsum delectus, quis suscipit iste, quisquam excepturi commodi, ipsam dolorum fugiat similique nostrum nisi earum dolores reiciendis id facere repudiandae voluptatum molestiae. Reprehenderit tempore, corrupti voluptates totam enim suscipit iure repellat sunt, laboriosam ipsam illo mollitia iusto magnam? Itaque, iusto. ',

  section2:
    'Lorem ipsum dolor sit amet consectetur adipisicing elit. Exercitationem magnam ipsum delectus, quis suscipit iste, quisquam excepturi commodi, ipsam dolorum fugiat similique nostrum nisi earum dolores reiciendis id facere repudiandae voluptatum molestiae. Reprehenderit tempore, corrupti volu ',

  section3:
    'Lorem ipsum dolor sit amet consectetur adipisicing elit. Exercitationem magnam ipsum delectus, quis suscipit iste, quisquam excepturi commodi, ipsam dolorum fugiat similique nostrum nisi earum dolores reiciendis id facere re Lorem ipsum dolor sit amet consectetur adipisicing elit.',
}

export const Projects = [
  {
    id: 1,
  
   description: `
    <div>
      <p>Achieved around SAR 3.2 million in revenues in 1 year, driving strategic growth initiatives.</p>
      <ul class="list-disc pl-5">
        <li>Led performance marketing efforts, optimizing Projects for impactful results.</li>
        <li>Collaborated with cross-functional teams to align strategies and achieve business objectives.</li>
      </ul>
      <p>Skills:</p>
      <p>Instagram Ads • Snapchat Ads • Conversion Tracking • Google Analytics • Dynamic Remarketing • Google Ads</p>
    </div>`,

  image: [
'https://i.postimg.cc/dtxhppnh/case-1.png'
  ],
  },

  {
    id: 2,
     description: `
<div class=" px-2 md:!px-6 shadow-md rounded-lg">
  <ul class="list-disc  space-y-2 ">
    <li><strong>Marketing Objectives : </strong>Conversion Campaign</li>
    <li><strong>Industry:</strong> E-commerce</li>
    <li><strong>Platform:</strong> TikTok</li>
    <li><strong>Country:</strong> Saudi Arabia</li>
    <li><strong>Total Purchases:</strong> 23k Orders</li>
    <li><strong>CPP (Cost Per Purchase):</strong> 24 SAR</li>
    <li><strong>Sales:</strong> 4,860,000 SAR</li>
    <li><strong>A.O.V:</strong> 209 SAR</li>
    <li><strong>ROAS:</strong> 9</li>
    <li><strong>Total Spend:</strong> 540,739.96 SAR</li>
  </ul>
</div>
    `,

  image: [
'https://i.postimg.cc/qvTDWZ5c/case-2.png'
  ],
  },
  {
    id: 3,

    description: `
 <div class="px-2 md:!px-6 shadow-md rounded-lg">
   <h2 class="font-bold text-xl mb-4 text-gray-800">Get +159 Orders in less than 6 days.</h2>
  <ul class="list-disc space-y-2">
    <li><strong>Marketing Objectives :</strong> Conversion Campaign</li>
    <li><strong>Industry:</strong> E-commerce</li>
    <li><strong>Platform:</strong> TikTok</li>
    <li><strong>Country:</strong> Saudi Arabia</li>
    <li><strong>Total Purchases:</strong> 159 Orders</li>
    <li><strong>CPP (Cost Per Purchase):</strong> 43 SAR</li>
    <li><strong>Sales:</strong> 74,800 SAR</li>
    <li><strong>A.O.V:</strong> 470 SAR</li>
    <li><strong>ROAS:</strong> 11</li>
    <li><strong>Total Spend:</strong> 6,800 SAR</li>
  </ul>
</div>
    `, 
  image: [
'https://i.postimg.cc/XXMdGmCs/case-3.png'
  ],
  },
  {
    id: 4,

    description: `
<div class="px-2 md:!px-6 shadow-md rounded-lg">
  <ul class="list-disc space-y-2">
    <li><strong>Marketing Objectives:</strong> Conversion Campaign</li>
    <li><strong>Industry:</strong> E-commerce</li>
    <li><strong>Platform:</strong> Instagram</li>
    <li><strong>Total Purchases:</strong> 381 Orders</li>
    <li><strong>CPP (Cost Per Purchase):</strong> 27 SAR</li>
    <li><strong>Sales:</strong> 178,479.00 SAR</li>
    <li><strong>A.O.V:</strong> 468 SAR</li>
    <li><strong>ROAS:</strong> 17</li>
    <li><strong>Total Spend:</strong> 10,600 SAR</li>
  </ul>
</div>
    `,
  image: [
'https://i.postimg.cc/855hqhGm/case-4.png'
  ],
  },
  {
    id: 5,

    description: `
<div class="px-2 md:!px-6 shadow-md rounded-lg">
  <ul class="list-disc space-y-2">
    <li><strong>Marketing Objectives:</strong> Conversion Campaign</li>
    <li><strong>Industry:</strong> E-commerce</li>
    <li><strong>Platform:</strong> Instagram</li>
    <li><strong>Total Purchases:</strong> 734 Orders</li>
    <li><strong>CPP (Cost Per Purchase):</strong> 7.46 USD</li>
    <li><strong>Sales:</strong> 53,108.65 USD</li>
    <li><strong>A.O.V:</strong> 72.35 USD</li>
    <li><strong>ROAS:</strong> 10</li>
    <li><strong>Total Spend:</strong> 5,550 USD</li>
  </ul>
</div>`
    ,  image: [
    'https://i.postimg.cc/jjjcbVQf/case-5.png',
    'https://i.postimg.cc/kGMXjcd7/case-6.png'
  ],

  },


]
